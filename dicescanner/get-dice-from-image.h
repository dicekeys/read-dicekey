...
static std::string getDiceJsonFromBoxImage(cv::Mat boxImage, std::string fileIdentifier = "", std::string intermediateImagePath = "/dev/null", int boxDebugLevel = 0) {
	static const std::vector<int> imageEncodingParametersForLosslessPNG = {cv::IMWRITE_PNG_COMPRESSION, 0};
	auto dieRead = getDiceFromImage(boxImage, fileIdentifier, intermediateImagePath, boxDebugLevel);
